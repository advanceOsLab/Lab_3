...
mem_init(void)
{
	...
	//cprintf("kern_pgdir: %x,kern_pgdir[PDX(UVPT)]: %x,PDX(UVPT): %x,PADDR(kern_pgdir): %x\n",kern_pgdir,kern_pgdir[PDX(UVPT)],PDX(UVPT),PADDR(kern_pgdir));
	//////////////////////////////////////////////////////////////////////
...
//>>>>>>> lab3
	...
	//How page 0 have IDT and BIOS structs.  
	...
	//size_t mpentry_i = PGNUM(MPENTRY_PADDR);
	for (i = 1; i < npages ; i++) 
	{
		if(i == PGNUM(MPENTRY_PADDR))  //This is same as MPENTRY_PADDR/PGSIZE 
		{
			//pages[i].pp_link = NULL;	//Lab 4
		}
...
mmio_map_region(physaddr_t pa, size_t size)
{
	...
	if(base>MMIOLIM)
	{
		panic("Mapping above allowed space");
	}
